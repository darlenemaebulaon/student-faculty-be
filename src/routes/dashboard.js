...
router.get('/appointments/:id', requireAuth, authorizeRoles('student','faculty'), async (req,res) => {
  try {
    ...
    if (appt.requester.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'Not authorized' });
    res.json(appt);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
router.put('/appointments/:id/cancel', requireAuth, authorizeRoles('student','faculty'), async (req,res) => {
  try {
    ...
    if (!appt) return res.status(404).json({ message: 'Appointment not found' });
    if (appt.requester.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'Not authorized' });
    if (appt.status === 'cancelled' || appt.status === 'completed') return res.status(400).json({ message: 'Cannot cancel' });
...
router.get('/records/:id', requireAuth, authorizeRoles('student','faculty'), async (req,res) => {
  try {
    ...
    if (rec.patient.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'Not authorized' });
    res.json(rec);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
